...
BUILDIN(partyinvite) {
    ...
    if( !sd ) {
        ShowError("script_partyinvite: no player attached.\n");
        script_pushint(st,0);
        return true;
    }
    
    party_id = script_hasdata(st,2)?script_getnum(st,2):0;
    p = party->search(party_id);
    
    if( !p ) {
        ShowError("script_partyinvite: party not found.\n");
        script_pushint(st,0);
        return true;
    }
    ...
    if( i >= MAX_PARTY ) {
        ShowError("script_partyinvite: party leader not found.\n");
        script_pushint(st,0);
        return true;
    }
    
    ARR_FIND(0, MAX_PARTY, j, p->party.member[j].account_id == 0);
    
    if( j >= MAX_PARTY ) {
        ShowError("script_partyinvite: chosen party is full!\n");
        script_pushint(st,0);
        return true;
    }
    
    if( sd->status.party_id || sd->party_invite || sd->party_joining) {
        ShowError("script_partyinvite: player already has a party!\n");
        script_pushint(st,0);
        return true;
    }
    ...
    script_pushint(st,1); //Everything should be good.
    return true;
}
